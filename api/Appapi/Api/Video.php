...
class Api_Video extends PhalApi_Api {
	...
	public function getRules() {
		return array(
            ...
            'setVideo' => array(
				...
				'goodsinfo' => array('name' => 'goodsinfo', 'type' => 'string', 'desc' => '商品信息，没有时为空字符串'),
			),
            'setComment' => array(
                'uid' => array('name' => 'uid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '用户ID'),
				'token' => array('name' => 'token', 'type' => 'string', 'require' => true, 'desc' => '用户Token'),
	...
	public function setVideo() {
        ...
        if($goodsinfo!=''){
            ...
            $ishsop = $domain2->isShop($uid);
            if($ishsop==0){
                $rs['code'] = 10001;
                $rs['msg'] = '您还未申请店铺，不能发布商品视频';
                return $rs;
            }
        
            $goods=json_decode($goodsinfo,true);
            if(isset($goods['name']) && isset($goods['href']) && isset($goods['thumb'])  && isset($goods['price']) && isset($goods['des']) ){
                if($goods['name']==''){
                    $rs['code'] = 1002;
                    $rs['msg'] = '请填写商品名称';
                    return $rs;
                ...
                if(strpos($goods['href'],'http')===false){
                    $rs['code'] = 1003;
                    $rs['msg'] = '请填写正确的商品链接';
                    return $rs;
                }
                
                $goods['href']=html_entity_decode($goods['href']);
        ...
        if($goods){
            ...
            $goods['uid']=$uid;
            $goods['videoid']=$info['id'];
            
            $domain2 = new Domain_Shop();
            $domain2->setGoods($goods);
