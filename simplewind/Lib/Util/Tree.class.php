...
class Tree {
    ...
    /**
     * 安全的字符串变量替换（替代危险的eval）
     * @param string $str 模板字符串
     * @param array $vars 变量数组
     * @return string
     */
    protected function safeReplace($str, $vars) {
        // 使用preg_replace_callback安全替换变量
        return preg_replace_callback('/\$([a-zA-Z_][a-zA-Z0-9_]*)/', function($matches) use ($vars) {
            $varName = $matches[1];
            return isset($vars[$varName]) ? htmlspecialchars($vars[$varName], ENT_QUOTES, 'UTF-8') : '';
        }, $str);
    }
    ...
    public function get_tree($myid, $str, $sid = 0, $adds = '', $str_group = '') {
        ...
        if (is_array($child)) {
            ...
            foreach ($child as $id => $value) {
                ...
                // 安全替换：移除危险的extract和eval
                $vars = array_merge($value, array('spacer' => $spacer, 'selected' => $selected, 'id' => $id));
                $parentid = isset($value['parentid']) ? $value['parentid'] : 0;
                $nstr = $this->safeReplace($parentid == 0 && $str_group ? $str_group : $str, $vars);
                $this->ret .= $nstr;
                $nbsp = $this->nbsp;
    ...
    public function get_tree_multi($myid, $str, $sid = 0, $adds = '') {
        ...
        if (is_array($child)) {
            ...
            foreach ($child as $id => $a) {
                ...
                // 安全替换：移除危险的extract和eval
                $vars = array_merge($a, array('spacer' => $spacer, 'selected' => $selected, 'id' => $id));
                $nstr = $this->safeReplace($str, $vars);
                $this->ret .= $nstr;
    ...
    public function get_tree_category($myid, $str, $str2, $sid = 0, $adds = '') {
        ...
        if (is_array($child)) {
            ...
            foreach ($child as $id => $a) {
                ...
                // 安全替换：移除危险的extract和eval
                $vars = array_merge($a, array('spacer' => $spacer, 'selected' => $selected, 'id' => $id));
                $html_disabled = isset($a['html_disabled']) ? $a['html_disabled'] : '';
                if (empty($html_disabled)) {
                    $nstr = $this->safeReplace($str, $vars);
                } else {
                    $nstr = $this->safeReplace($str2, $vars);
                }
                $this->ret .= $nstr;
    ...
    function get_treeview($myid, $effected_id='example', $str="<span class='file'>\$name</span>", $str2="<span class='folder'>\$name</span>", $showlevel = 0, $style='filetree ', $currentlevel = 1, $recursion=FALSE) {
        ...
        foreach ($child as $id => $a) {
            ...
            $vars = array_merge($a, array('id' => $id, 'name' => $name));
        ...
        if (is_array($sub_cats))
            foreach ($sub_cats as $c) {
                ...
                if ($this->get_child($c['catid'])) {
                    ...
                    if ($str) {
                        // 安全替换：移除危险的extract和eval
                        $vars = function_exists('array_iconv') ? array_iconv($c, CHARSET, 'utf-8') : $c;
                        $data[$n]['text'] = $this->safeReplace($str, $vars);
                    } else {
                        $data[$n]['text'] = iconv(CHARSET, 'utf-8', $c['catname']);
                    }
                }
