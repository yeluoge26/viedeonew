...
class UeditorController extends Controller {
	...
	public function uploadimg(){
		...
		if(isset($_FILES['upfile']) && is_uploaded_file($_FILES['upfile']['tmp_name'])){
			...
			$isValidImage = false;
			foreach($allowedMimes as $magic => $mime){
			...
			if(!$isValidImage){
				$response = "{'url':'','title':'','original':'','state':'ERROR_TYPE_NOT_ALLOWED: 文件内容不是有效的图片'}";
				exit($response);
			}

			// 检查是否包含PHP代码（防止图片马）
			$content = file_get_contents($tmpFile);
			if(preg_match('/<\?php|<\?=|<%|eval\s*\(|base64_decode/i', $content)){
				$response = "{'url':'','title':'','original':'','state':'ERROR_TYPE_NOT_ALLOWED: 文件包含非法内容'}";
				exit($response);
			}
		}
		...
		$config=array(
				...
				'mimes'      =>    array('image/jpeg', 'image/png', 'image/gif', 'image/bmp'), // 添加MIME类型验证
				'autoSub'    =>    false,
		);
	...
	private function _ueditor_upload($config=array()){
		...
		$upload_type=$configpri['cloudtype'];
