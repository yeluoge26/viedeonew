 ...
 * 上热门支付回调
 */
class PopularbackController extends HomebaseController {
	...
	public function notify_ali() {
		...
		if($verify_result) {//验证成功
			...
			if($_POST['trade_status'] == 'TRADE_FINISHED') {
                ...
                $where['status']=0;
                $where['type']=1;
				$orderinfo=M("popular_orders")->where($where)->find();	
				$this->logali("orderinfo:".json_encode($orderinfo));	
				if($orderinfo){
					/* 更新视频信息 */
					$nums=$orderinfo['nums'];
					$expire=time() + $orderinfo['length']*60*60;
					$uid=$orderinfo['uid'];
					$videoid=$orderinfo['videoid'];
                    
                    M("users_video")->where("uid='{$uid}' and id={$videoid}")->save(array("p_nums"=>$nums,"p_expire"=>$expire,"p_add"=>time()));

					/* 更新 订单状态 */
					M("popular_orders")->where("id='{$orderinfo['id']}'")->save(array("status"=>1,"trade_no"=>$trade_no));
	...
	private function orderServer(){
		...
		if($orderinfo){
			...
			/* 更新 订单状态 */
			M("popular_orders")->where("id='{$orderinfo['id']}'")->save(array("status"=>1,"trade_no"=>$info['transaction_id']));
	...
	*/
	...
	public function logali($msg){
		file_put_contents(SITE_PATH.'data/paylog/popular_logali_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').'  msg:'.$msg."\r\n",FILE_APPEND);
	}
	/* 打印log */
	public function logwx($msg){
		file_put_contents(SITE_PATH.'data/paylog/popular_logwx_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').'  msg:'.$msg."\r\n",FILE_APPEND);
	}
