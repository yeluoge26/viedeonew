...
class PageController extends HomebaseController{
	...
	public function pagecon() {
		$id=intval($_GET['id']);
		$page=M("page")->where(array('id'=>$id))->find();
	...
	public function newslists() {
		 ...
		 $name=M("news_class")->where(array('id'=>$term_id))->getField("title");

			$list=M("news")
					->field("id,title")
					->where(array('classid'=>$term_id))
					->order("listorder asc")
					->select();
	...
	public function news() {
     $id=intval($_GET['id']);
		$news=M("news")->field("title,body")->where(array('id'=>$id))->find();
