...
class PayController extends HomebaseController {
	...
	public function notify_ali() {
		...
		if($verify_result) {//验证成功
			...
			if($_POST['trade_status'] == 'TRADE_FINISHED') {
				...
				// 安全过滤：防止SQL注入
				$safe_out_trade_no = addslashes(trim($out_trade_no));
				$safe_total_fee = floatval($total_fee);
				$orderinfo=M("users_charge")->where(array(
					'orderno'=>$safe_out_trade_no,
					'money'=>$safe_total_fee,
					'status'=>0,
					'type'=>1
				))->find();
				...
				if($orderinfo){
					...
					M("users")->where(array('id'=>intval($orderinfo['touid'])))->setInc("coin",$coin);
					/* 更新 订单状态 */
					M("users_charge")->where(array('id'=>intval($orderinfo['id'])))->save(array("status"=>1,"trade_no"=>addslashes($trade_no)));
	...
	private function orderServer(){
		...
		// 安全过滤：防止SQL注入
		$safe_out_trade_no = addslashes(trim($info['out_trade_no']));
		$orderinfo=M("users_charge")->where(array(
			'orderno'=>$safe_out_trade_no,
			'status'=>0,
			'type'=>2
		...
		if($orderinfo){
			...
			M("users")->where(array('id'=>intval($orderinfo['touid'])))->setInc("coin",$coin);
			/* 更新 订单状态 */
			M("users_charge")->where(array('id'=>intval($orderinfo['id'])))->save(array("status"=>1,"trade_no"=>addslashes($info['transaction_id'])));
		}else{
	...
	*/
	...
	public function notify_ios(){
		...
		// 安全过滤：防止SQL注入
		$safe_transaction_id = addslashes(trim($info['transaction_id']));
		$safe_out_trade_no = addslashes(trim($out_trade_no));
		$iforderinfo=M("users_charge")->where(array(
			'trade_no'=>$safe_transaction_id,
			'type'=>3
		))->find();
		...
		if($orderinfo){
			...
			M("users")->where(array('id'=>intval($orderinfo['touid'])))->setInc("coin",$coin);
			/* 更新 订单状态 */
			M("users_charge")->where(array('id'=>intval($orderinfo['id'])))->save(array("status"=>1,"trade_no"=>$safe_transaction_id));
