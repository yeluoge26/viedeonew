...
class PageController extends HomebaseController{
   ...
   public function test11111111111111111() {
       $userinfo=M("users")->field("id,avatar")->select();
			 foreach($userinfo as $k=>$v){
				    if(strstr($v['avatar'],'http://api.phpweb.cn:8080/' )){
							  ...
							  $newavatar=str_replace("http://api.phpweb.cn:8080/","https://api.semonghuang.org/public/",$v['avatar']);
	...
	public function questions(){
		...
		//读取常见问题下的文章列表
		$list=M("term_relationships")->order("listorder")->where("term_id=13")->select();
		$questionList=array();
		foreach ($list as $k => $v) {
