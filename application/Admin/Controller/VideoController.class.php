	...
	function add_post(){
		if(IS_POST){			
			...
			if($url!=""){
                ...
                if(strtolower($video_type_w)!='mp4'){
                    $this->error("文件名后缀必须为mp4格式");
                }
                $video->href_w=$url_w;
                ...
                $src_w=$files_w["tmp_name"];
                $type_w='video';
                
                $rs_w=adminUploadFiles($files_w,$src_w,$type_w);
                if($rs_w['code']!=0){
                   $this->error($rs_w['msg']);
                }
                
				$video->href=$rs['data']['url'];
				$video->href_w=$rs_w['data']['url'];
			}
